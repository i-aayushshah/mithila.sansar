...
import { X, Loader, Palette } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { fetchChatbotResponse } from '@/lib/chatbotApi';

export default function ArtFormDialog({ isOpen, onClose, artForm }) {
  ...
  useEffect(() => {
    if (isOpen && artForm) {
      setLoading(true);
      fetchChatbotResponse(`Tell me more about ${artForm.name}, a traditional art form from Mithila.`)
        .then(response => {
          setDetails(response);
          setLoading(false);
        })
        .catch(error => {
          console.error('Error fetching art form details:', error);
        ...
        >
          ...
          >
            <div className="relative h-64 flex-shrink-0">
              <Image
                src={artForm.image || '/images/default-art-form.jpg'}
                alt={artForm.name}
                layout="fill"
                objectFit="cover"
              />
            ...
            <div className="flex-grow overflow-y-auto p-8 custom-scrollbar">
              <div className="flex items-center text-gray-600 mb-6">
                ...
                <span>Traditional Mithila Art Form</span>
              </div>
