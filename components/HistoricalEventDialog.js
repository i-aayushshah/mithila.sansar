...
export default function HistoricalEventDialog({ isOpen, onClose, event }) {
  ...
  useEffect(() => {
    if (isOpen && event) {
      setLoading(true);
      fetchChatbotResponse(`Tell me more about ${event.title}, a significant historical event in Mithila.`)
        .then(response => {
          setDetails(response);
          setLoading(false);
        })
        .catch(error => {
          console.error('Error fetching historical event details:', error);
